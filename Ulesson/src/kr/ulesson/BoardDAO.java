...
public class BoardDAO {
	...
	public void MyBoard(String memId) {
		...
		ResultSet rs = null;
		String sql = "SELECT bd_num, bd_content FROM board WHERE mem_id = ? ORDER BY bd_num ASC";
		...
		try {
			...
			if (!rs.next()) {
				...
				do {
					...
					System.out.println("------------------------------------------");
					System.out.println("번호 : " + bdNum);
					System.out.println("내용 : " + bdContent);
	...
	// 글 수정
	public void updateBoard(int bdNum, String newContent, int newCategory) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = null;
		...
		try {
			if (!isBDCT_numExist(newCategory)) {
				System.out.println("유효하지 않은 카테고리 번호입니다. 수정이 불가능합니다.");
				return;
			}
			conn = DBUtil.getConnection();
			// 게시글 내용 수정
			sql = "UPDATE board SET bd_category = ?, bd_content = ? WHERE bd_num = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, newCategory);    // 수정된 카테고리 번호
			pstmt.setString(2, newContent);  // 수정된 내용			
			pstmt.setInt(3, bdNum);

			int count = pstmt.executeUpdate();

			if (count > 0) {
				System.out.println("게시글 " + bdNum + "번이 성공적으로 수정되었습니다.");
			} else {
				System.out.println("게시글 수정에 실패했습니다. 해당 번호의 게시글이 존재하는지 확인하세요.");
			}
		}catch (Exception e) {
			e.printStackTrace();
		} finally {
